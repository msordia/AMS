...
def viajeActualTaxista(idTaxi, cursor):
	query = "SELECT p.nombre, v.Id_viaje, v.Origen, v.Destino, v.FechaYHora, v.Costo, t.Marca, t.Modelo, t.Placas, t.Color FROM Viaje v JOIN Taxi t ON v.Id_taxi = t.Id_taxi JOIN Persona p ON v.Id_cliente = p.id_persona  WHERE t.id_taxista = " + str(idTaxi)  + " AND v.Estatus = 0;"
	cursor.execute(query);
	result = cursor.fetchone();
	if result == None:
		return 'none';
